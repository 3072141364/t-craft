...
| [mattpocock/skills](https://github.com/mattpocock/skills) | code-review:双轴(Standards/Spec)+ Fowler 异味基线 + 两轴分开报告。**后续 TODO skill(brainstorm / eval-report / dev-flow)着重参考此仓库**:`skills/engineering/` 下的 research、grill-with-docs、to-spec、triage、domain-modeling、tdd、diagnosing-bugs 可直接借鉴 |
...
| 仓库 | 用途 |
|------|------|
| [nvm-sh/nvm](https://github.com/nvm-sh/nvm) | Node 版本管理(code-intelligence/setup_env.sh、scripts/setup-ubuntu.sh) |
| [astral-sh/uv](https://github.com/astral-sh/uv) | Python 环境管理(code-format/setup_format_tools.sh、scripts/setup-*.sh) |
...
| 仓库 | 用途 |
|------|------|
| [abhigyanpatwari/GitNexus](https://github.com/abhigyanpatwari/GitNexus) | 代码知识图谱 MCP;code-intelligence 路由的深度分析工具;`gitnexus-*` 子 skill 由其 `analyze` 生成 |
| codegraph(npm 包) | 代码调用图 CLI;code-intelligence 路由的日常工具 |

## 官方插件库

| 仓库 | 用途 |
|------|------|
| [anthropics/claude-plugins-official](https://github.com/anthropics/claude-plugins-official) | Claude 官方插件 marketplace;code-intelligence 引用其 `*-lsp` 插件(pyright-lsp、clangd-lsp 等) |
