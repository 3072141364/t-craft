---
description: 代码审查(影响面评估 / 全量评估两档),双轴(Standards 规范 / Spec 需求)。/tcraft-code-review = 全量评估(未提交);/tcraft-code-review impact = 影响面评估(轻量);/tcraft-code-review full = 全量评估;/tcraft-code-review <base..head> = 指定范围。VCS 无关(纯 git),出报告,不审格式(交 code-guidelines 附属文档 format / make lint)。
...
- **影响面评估**(`impact`):**轻量档**。范围=未提交(可 `-- <base..head>` 指定);跑阶段一(范围/上下文)+ **影响面分析**(code-intelligence `impact` / `trace`:改动影响哪些调用链/模块,确认不破坏其他功能)+ Spec 轴需求达成轻确认。**开发完第一步、发布前快速确认用**。
- **全量评估**(`full` / 无参数):**全面档**。范围=未提交(`git diff` + `git diff --cached`);跑阶段 1 / 2 / 3 / 4(完整双轴审查,含高层架构/性能/测试策略)。验证阶段主体、合并前用。
- `<base..head>`(如 `main..HEAD`、`v1.0..HEAD`)-> 指定范围,按全量评估维度。

主干自动探测:`origin/main` / `main` / `master`。**先验证 fixed point 解析得了、diff 非空**(`git rev-parse` + `git diff --stat`),空或无效就停,别进后续。无 git / 范围无效 -> 停并告知。

## 不审什么

格式 / 风格 / 导入排序 / 拼写 / lint / 类型检查全交 code-guidelines 附属文档(format / make-shortcut)+ `make lint`(ruff / mypy / clang-format / clang-tidy 能查的不审)。预存技术债不审。**只审本次改动的行。**
...
3. **识别规范来源(Standards 轴用)**:根 CLAUDE.md + 改动目录的 CLAUDE.md + `CODING_STANDARDS.md` / `CONTRIBUTING.md` 等;**外加代码异味基线**(读 `../code-guidelines/references/code-smells.md`,Fowler 12 味,judgement call,项目规范覆盖基线,工具能查的跳过)。
...
- 代码注释约定:违反文件内注释里的约定。
- 代码异味:命中 `../code-guidelines/references/code-smells.md` 的 12 味(命名 / 重复 / Feature Envy / Data Clumps / Primitive Obsession / Shotgun Surgery / …),标"可能 X",judgement call,项目规范覆盖。
- 历史上下文:`git blame` 看是否回退既有修复、与历史意图冲突。
- 结构化注释标记:改动里遇 `TODO`/`FIXME`/`HACK`/`XXX`/`REVIEW` 等标记,按 `../code-guidelines/references/code-markers.md` 判断——本次改动**新引入**的 `FIXME`/`XXX`(已知缺陷/可疑)该报;预存的 `TODO` 不报(预存技术债,不在本次范围)。
...
语言专项:审到 bash / python / cpp 时,读对应语言 skill 的 `references/review.md`(bash-guidelines / python-guidelines / cpp-guidelines)拿该语言常见 bug + 工具管不到的逻辑问题(主要喂 Spec 轴)。
...
- **不绑 git-guidelines**:review -> 改 -> 再 review -> 没问题 -> 用户手动跑 git-guidelines 提交。本命令不触发 git-guidelines。
