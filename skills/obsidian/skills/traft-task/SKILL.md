...
用 PARA 的 `project/` 下任务 note 管理，靠 `task-component.components` 多视图看进度。**一个任务 = 一篇带 `task` tag 的 note**。
...
## 任务视图（`task-component.components`）
- 视图在 **`archive/component/`**：`task-component.components`（不在 vault 根），含 表格 / 看板(按 priority) / 日历(start) / 画廊 / 甘特(start→doneTime)。
- 只在 vault 有 `task-component.components` 时可见；换电脑把插件 `templates/task-component.components` 拷到 `archive/component/`。
