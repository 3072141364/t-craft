---
name: changelog
description: 维护项目 CHANGELOG,把代码变更转化为规范化的变更记录。当用户要求"更新 changelog""写变更记录""整理本次改动""把 unreleased 收尾""发版了整理变更"时使用。遵循 Keep a Changelog 格式,类别标题带 emoji。项目特定规则(CHANGELOG 路径、版本真源、类别细则)在运行时从项目 CLAUDE.md / 标准文件发现,不硬编码。
---

# Changelog

把代码变更转化为面向消费者的变更记录,按 [Keep a Changelog](https://keepachangelog.com/) 维护。

## 何时触发

- **提交代码时**:维护 `[Unreleased]` 段。
- **用户直接要求**:写变更记录、整理本次改动。
- **发版收尾**:把 `[Unreleased]` 转为正式版本条目。
- **批量整理**:从某段提交历史生成 changelog。
...
- **不擅自动版本真源**:版本号递增由用户或提交流程决定,本 skill 只在发版收尾时读版本号,不改它。
