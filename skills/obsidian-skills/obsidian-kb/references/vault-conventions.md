...
> **1个单词或专有名词 → 英文；2个词以上 → 中文；中文名中遇术语/专有名词保留英文，否则不中英混写。**

| 情况 | 规则 | 示例 |
|------|------|------|
| 1个单词 / 专有名词 | 英文 | `overlay`、`GSServer`、`e2-radar` |
| 2个词以上（短句） | 中文 | `安装环境残留重试必败` |
| 中文名中遇术语/专有名词 | 保留英文，其余中文 | `overlay挂载只读失败排查`、`GSServer独占式资源租借调度器` |
| 其他 | 不中英混写 | ❌ `install环境残留重试必败` → ✅ `安装环境残留重试必败` |

### 各类型格式

| 类型 | 格式 | 示例 |
|------|------|------|
| 目录 | 按总原则 | `projects/<项目名>/` |
| 需求文件夹 | 按总原则 | `overlay` / `安装环境残留重试必败` / `GSServer独占式资源租借调度器` |
