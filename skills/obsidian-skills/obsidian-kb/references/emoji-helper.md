...
> obsidian-kb 的附属文档:emoji 选择与应用(含速查表)。给 commit / CHANGELOG / 卡片标题 / 周报字段 / PR 标题配 emoji、或问"该用哪个 emoji"时读本文;完整速查表见同目录 `emoji-cheatsheet.md`。

给一段内容选出语义正确的 emoji、按规范放到正确位置、校验用法合规。其他 skill 不再各自维护映射表,统一向本文查询。规则是通用默认;项目特有约定从项目 CLAUDE.md 发现。

## 规范依据

- **通用默认规则**:见下文「核心原则」与「高频映射」。这是跨项目的合理默认。
- **项目特有约定**:读项目 `CLAUDE.md`,看是否有项目专属的 emoji 规则(如自定义类别 emoji、禁用某些 emoji)。有则覆盖默认。
- **完整速查表**:长尾场景查 `emoji-cheatsheet.md`(随本文附带,不依赖项目)。

## 何时触发

- **用户直接问**:选 emoji、给文字配 emoji、确认某 type/类别对应哪个 emoji。
- **写 commit / CHANGELOG / PR 标题 / 状态标注 / 文档笔记**时,需要 emoji 前缀或点缀。
- **其他 skill 委托**:git-guidelines 写 commit/PR 标题与 CHANGELOG 条目时,emoji 选择交本文。
...
3. **长尾查速查表**:不在高频表里,读 `emoji-cheatsheet.md` 对应章节。
4. **校验规则**:用核心原则逐条过。
...
2. 按位置规则放置。
3. 若与其他 skill 协作(如 git-guidelines 已给出 commit type 或 CHANGELOG 类别),直接据其输入查表返回 emoji。
4. 输出可直接落笔的文字。

## 与其他 skill 的协作

本文是 emoji 的唯一映射来源,其他 skill **不再硬编码映射表**:

- **git-guidelines**:写 CHANGELOG 条目时,类别 emoji 与条目 emoji 向本文查询。
- **git-guidelines**:写 commit 信息 / PR 标题时,type->gitmoji 向本文查询。
- **任意 skill**:需要给输出文字加 emoji 时,调用本文。
