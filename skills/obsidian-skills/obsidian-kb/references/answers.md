...
> obsidian-kb 的附属文档:快速问答(已知答案索引 + 自动学习)。高频固定操作/约定问题,先查索引直接答,未命中转查询,搜到学习。
...
设计意图:obsidian-query 解决"从零搜"(grep+summary 3-tier 读),但没缓存——同一个固定问题每次都从头搜。本文补一层**已知答案索引**,把高频固定问题的"问题→文档路径→章节"记下来,命中 1 次 Read 直接答,省掉粗筛+分级读的整条链路。索引内容是项目特定的(哪个问题→哪篇文档),放 vault 项目文件夹随 vault 跨机器同步;机制是通用的(查索引→命中答→未命中搜→学),放本文。

## 何时触发

- **固定/常问的操作约定问题**:用户问"X命令是什么""X怎么做""构建/发版/部署/初始化/打包怎么搞""之前查过的X"——答案是某篇文档已写好的固定操作。
- **被其他 skill 调用**:obsidian-kb 模块二查文档前先调本文,命中跳过搜索。
...
2. **发现项目名**:当前 git 仓库目录名 → 匹配 vault `projects/` 下文件夹;匹配不到读 CLAUDE.md 推断;都没有问用户。
3. **定位索引文件**:`<vault>/projects/<项目名>/answer-index.jsonl`。不存在则首次写时创建(目录已在 init_vault 建好)。
...
```json
{"q":"构建镜像命令","tags":["build","docker","镜像"],"doc":"projects/<项目名>/wiki/<发版流程文档>.md","section":"重建镜像","summary":"bash script/build.sh(默认不带后缀),build后手动push","hits":0,"updated":"2026-08-10"}
```
...
```bash
# 读索引(不存在则首次查时降级到搜索)
cat "<vault>/projects/<项目名>/answer-index.jsonl" 2>/dev/null || echo "__NO_INDEX__"
```
...
索引无命中或低置信,转模块二(query.md)做 3-tier 省 token 搜索:粗筛 → 按 summary 分级读 → 沿链接扩展。本文不重复实现搜索逻辑。

返回结果按 query.md 的分级格式(强/弱/无关)。
...
obsidian-kb 模块二、其他 skill 查文档前可先调本文:

1. 调本文查索引。
2. **命中**:返回 `doc`+`section`+`summary`,调用方直接 Read 对应段,跳过搜索。
3. **未命中**:返回"未命中",调用方自行走 obsidian-query 搜;搜到后可再调本文的"学习"步骤记索引。

调用方约定:本文返回结构化结果(命中条目或"未命中"),不替调用方组织回答。
...
- **obsidian-query**:本文未命中时转调它搜;它是纯搜索无缓存,本文是其上层的命中缓存层。两者互补:query 解决"从零搜",answers 解决"已搜过的直接答"。
- **obsidian-kb**:模块二查文档前可先调本文;本文的学习步骤(写 jsonl)与 obsidian-kb 的写卡(写 .md)不冲突,各管各的存储。
- **brainstorm / project-doc**:查旧方案/约定时可先调本文,命中省一次搜索。
...
- **零项目特定信息**:本文仓库不含任何具体项目、命令、路径;所有项目特定内容在 vault 的 answer-index.jsonl。
...
- 项目约定发现三层写法:本仓库 code-guidelines / git-guidelines skill(含附属文档)。
